...
class OfficeEquipment(Stock):
    ...
    '''добавим исключений, можно по аналогии и в словарь такое сделать'''
    def __init__(self, type_of_OE, model, price, presence, needs):
        ...
        try:
            ...
            self.needs = int(needs)
        except ValueError:
            print('неправильный ввод. Укажите числительные цифрами')
    ...
    def reception(self, kind_of_office, office_needs):
        ...
        if int(self.value_2) > int(self.presence):
            dict_of_presence_new = {
                'передача товара по месту требования в единицах': 'нет необходимого количества товара'}
